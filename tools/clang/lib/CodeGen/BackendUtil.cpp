...
//SWITCH FOR IR-LEVEL MUTATION GENERATION
#if ACCMUT_GEN_MUT
    wrapper = new MutationGen();
#endif

//SWITCH FOR MUTATION SCHEMATA
#if ACCMUT_MUTATION_SCHEMATA
    workPass = new MSInstrumenter();
    wrapper = new FunctionPassToModulePassWrapper(workPass);
#endif

//SWITCH FOR DYNAMIC ANALYSIS
#if ACCMUT_DYNAMIC_ANALYSIS_INSTRUEMENT
    wrapper = new DMAInstrumenter();
#endif

//SWITCH FOR SOME STATISTICS
#if ACCMUT_STATISTICS_INSTRUEMENT
    workPass = new ExecInstNums();
    wrapper = new FunctionPassToModulePassWrapper(workPass);
#endif

#if ACCMUT_WINDOW_ANALYSIS_INSTRUMENT
    wrapper = new WAInstrumenter();
#endif
    // PerFunctionPasses.add(workPass);
    PerModulePasses.add(new AAResultsWrapperPass());
    PerModulePasses.add(new MemoryDependenceWrapperPass());
