...
void MutationGen::genROR(Instruction *inst, StringRef fname, int index) {
    ...
    for (int i = 0;
         i < (int) (CI->getNumOperands()); i++) {// if is pointer type , only change "!=" to "==" , "==" to "!="
...
static RegisterPass<MutationGen> X("AccMut-gen-mut", "AccMut - Generate Mutation");
/*-----------------reserved end --------------------*/
