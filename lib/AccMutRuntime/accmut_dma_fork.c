...
// TODO: array size
int forked_active_set[MAXMUTNUM];
int forked_active_num;
int default_active_set[MAXMUTNUM + 1];
static int recent_set[MMPL];
static int recent_num;
static long temp_result[MMPL];
...
void __accmut__divide__eqclass_cmp(int onlyhas_0, int onlyhas_1) {
    ...
    eqclass[0].num = 1;
    // TODO: DONE. initialize eqclass[0]
    eqclass[0].mut_id[0] = MUTATION_ID;
...
int __accmut__process_i32_cmp(int from, int to, int left, int right){
    ...
    /*if(MUTATION_ID == 0)
    {
        fprintf(stderr, "from:%d to:%d left:%d right:%d s_pre:%d ori:%d\n", from, to, left, right, s_pre, ori);
        fflush(stderr);
    }*/
